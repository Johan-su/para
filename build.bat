...
clang++ -MJ build/build_meta.json src/build_meta.cpp -o build/metagen.exe -DGIT_HASH=0x%GIT_HASH% %FLAGS% %WARNINGS% %WINLIBS% 
.\build\metagen.exe
echo Building src...
clang++ -MJ build/build.json src/build.cpp -o build/para.exe -DGIT_HASH=0x%GIT_HASH% %FLAGS% %INCLUDES% %WARNINGS% %LIBS% %WINLIBS% 

echo [ > compile_commands.json
type build\build_meta.json >> compile_commands.json
